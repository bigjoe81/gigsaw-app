...
export class BandPressKitService {
  ...
  async sharePdf(id: number, title?: string | null): Promise<void> {
    ...
    if (Capacitor.isNativePlatform()) {
      const uri = await this.writeNativeFile(blob, fileName, Directory.Cache);
      await Share.share({
        title: title ?? 'Press Kit',
        dialogTitle: 'Condividi press kit',
        files: [uri],
      });
      return;
    }
    ...
    if (nav.share && (!nav.canShare || nav.canShare({ files: [file] }))) {
      await nav.share({ title: title ?? 'Press Kit', files: [file] });
      return;
    }
  ...
  private fileName(title?: string | null): string {
    const safe = (title ?? 'press-kit')
      .toLowerCase()
      .trim()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '');

    return `${safe || 'press-kit'}.pdf`;
  }
}
