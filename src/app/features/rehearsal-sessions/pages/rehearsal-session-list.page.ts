...
export class RehearsalSessionListPage implements OnInit {
 ...
 load(event?:CustomEvent):void{if(!event)this.loading.set(true);this.error.set('');this.api.list().pipe(timeout(15000),finalize(()=>{this.loading.set(false);event?.detail.complete();})).subscribe({next:s=>this.sessions.set([...s].sort((a,b)=>String(a.date).localeCompare(String(b.date)))),error:(e:Error)=>this.error.set(e.message||'Impossibile caricare le prove.')});}
 formatDate(date?:string|null):string{return date?new Intl.DateTimeFormat('it-IT',{weekday:'short',day:'2-digit',month:'short'}).format(new Date(`${String(date).slice(0,10)}T00:00:00`)):'Data da definire';}
 time(s:RehearsalSession):string{return [s.startTime,s.endTime].filter(Boolean).map(value=>String(value).slice(0,5)).join(' – ')||'Orario da definire';}
}
